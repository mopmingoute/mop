 ...
 *
 * ListaCombustiblesController is part of MOP.
 *
...
public class ListaCombustiblesController extends ListaParticipante {
    ...
    private DatosCombustiblesCorrida datosCombustiblesCorrida;
    private Hashtable<String, DatosCombustibleCorrida> listaDatosCombustibleCorrida;
    private HashMap<Integer, List<Node>> nodesByRow = new HashMap<>();
    private Manejador parentController;
    private ListaCombustiblesController thisController;
    private String combustibleABorrar;
    private int indCombustibleABorrar;

    public ListaCombustiblesController(DatosCorrida datosCorrida, Manejador parentController) {
        super(datosCorrida,parentController);
        this.datosCorrida = datosCorrida;
        this.datosCombustiblesCorrida = datosCorrida.getCombustibles();
        this.listaDatosCombustibleCorrida = datosCorrida.getCombustibles().getCombustibles();
...
//        unloadData();
    ...
    /**
     *  Método para cargar los datos en el DatosCombustiblesCorrida
     */
    public void loadData(){
        System.out.println("LOAD DATA-LISTA COMBUSTIBLES");
        datosCombustiblesCorrida.setCombustibles(listaDatosCombustibleCorrida);
        datosCorrida.setCombustibles(datosCombustiblesCorrida);
    }

    /**
     * Método para obtener los datos del DatosCombustiblesCorrida y ponerlos en la interfaz (solo en modo edición)
     */
    public void unloadData() {
        System.out.println("UNLOAD DATA-LISTA COMBUSTIBLES");
        ArrayList<JFXCheckBox> checkBoxes = new ArrayList<>();
        for(DatosCombustibleCorrida datosCombustibleCorrida : listaDatosCombustibleCorrida.values()){
            Label nombre = new Label(datosCombustibleCorrida.getNombre());
            if(datosCombustibleCorrida.controlDatosCompletos().size() > 0){
                nombre.setText(datosCombustibleCorrida.getNombre() + " (*)");
            ...
            visible.setOnAction(action -> {
                parentController.setVisibleParticipate(datosCombustibleCorrida.getNombre(), visible.isSelected());
            ...
            editButton.setOnAction(actionEvent -> parentController.openEditor("EditorCombustible.fxml", "Editor Combustible - " +datosCombustibleCorrida.getNombre(), Text.COMBUSTIBLE_SIZE[0], Text.COMBUSTIBLE_SIZE[1], new EditorCombustibleController(datosCorrida, datosCombustibleCorrida, thisController, false)));
            cloneButton.setOnAction(actionEvent -> {
                DatosCombustibleCorrida clonado = new DatosCombustibleCorrida("Combustible clonado", datosCombustibleCorrida.getUnidad(), datosCombustibleCorrida.getPciPorUnidad(),
                        datosCombustibleCorrida.getDensidad(),datosCombustibleCorrida.isSalDetallada());
                parentController.openEditor("EditorCombustible.fxml", "Editor Combustible - " + clonado.getNombre(),  Text.COMBUSTIBLE_SIZE[0], Text.COMBUSTIBLE_SIZE[1], new EditorCombustibleController(datosCorrida, clonado, thisController, true));
                datosCombustiblesCorrida.getOrdenCargaXML().add("Combustible clonado");
                datosCombustiblesCorrida.getListaUtilizados().add("Combustible clonado");

            });
            deleteButton.setOnAction(actionEvent -> {

                combustibleABorrar = datosCombustibleCorrida.getNombre();
                indCombustibleABorrar = index;
                super.mostrarMensajeConfirmacionBorrado("Combustible " + combustibleABorrar);
            });
        }
    ...
    public void borrarCombustible(String combustibleABorrar){
        datosCombustiblesCorrida.getOrdenCargaXML().remove(combustibleABorrar);
        datosCombustiblesCorrida.getListaUtilizados().remove(combustibleABorrar);
        listaDatosCombustibleCorrida.remove(combustibleABorrar);
        unloadData();
        refresh();
    }
    @Override
    public void confirmoMensaje(boolean confirmacion, int codigoMensaje) {
        if(codigoMensaje == 2){
            if(confirmacion) {
                borrarCombustible(combustibleABorrar);
