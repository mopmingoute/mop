 ...
 *
 * EstimadorMarkovAmpliado2 is part of MOP.
 *
 ...
 *
 */
public class EstimadorMarkovAmpliado2 {
     ...
     * a ningún estado.
     * Primer índice estación, segundo índice serie
     */
    ...
    public void calculaVEContinuas(){
        ...
        CalculadorVE cVE = new CalculadorVE(cantSeries, nombresSeries, nombreDurPaso, 
        		nombresVE, cantVE, defVarEst,
    			cantRezagos, estandarizada, seriesLeidas,			
    			gruposDeVE, grupoDeVESerie,
    			operadorEnVE, ponderadorDeGruposEnVE);
    	varsEstado = cVE.construyeVE();
    }
    ...
    public void calculaMatricesYObservaciones(){
    	...
    	// Incorporación de las ventanas a las matrices
    	...
    	// Incorporación de los entornos a las observaciones
    	for(int ip=1; ip<= cantMaxPasos; ip++){
    		for(int iv = -radioEntorno; iv<=radioEntorno; iv++){
    			int ipiv = estadoCompuesto.avancePasoCircular(ip, iv); 
    			for(int iec=0; iec<cantEstComp; iec++){
    ...
    public void imprimeTextosParaDatatypesAmp(String directorio){
        ...
        /**
         * Genera salida de las matrices de transición
         * en MatTrans.xlt
         */
