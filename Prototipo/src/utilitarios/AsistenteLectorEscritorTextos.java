 ...
 *
 * AsistenteLectorEscritorTextos is part of MOP.
 *
 ...
 *
 */
public class AsistenteLectorEscritorTextos {
	...
	/**
	 * Este contructor se usa para las lecturas
	 * @param texto
	 * @param dirArchivo
	 */
	public AsistenteLectorEscritorTextos(ArrayList<ArrayList<String>> texto, String dirArchivo) {
		super();
		this.texto = texto;
	...
	/**
	 * Este constructor se usa para las escrituras
	 */
	public AsistenteLectorEscritorTextos(){};
	...
	/**
	 * Carga un booleano según el String leído en la columna 1 de la fila i.
	 * Si la columna 0 no coincida con la etiqueta detiene la ejecución.
	 * Si el String leído es "SI" o "TRUE" con mayúscula o minúscula carga el valor booleano true
	 * idem NO y FALSE
	 ...
	 * Si la columna 0 no coincide con la etiqueta detiene la ejecución.
	 * @param i
	 * @param etiqueta
	 * @return
	 */
	...
	/**
	 * Carga una lista de reales tomada de la fila i, de palabras a partir de la columna 1
	 * Si la columna 0 no coincide con la etiqueta detiene la ejecución.
	 * @param i
	 * @param etiqueta
	 * @return
	 */
	public ArrayList<Double> cargaListaReales(int i, String etiqueta){
		verificaEtiqueta(i, etiqueta);
		ArrayList<Double> aux = new ArrayList<Double>();
		for(int j=1; j<texto.get(i).size(); j++){
			aux.add(Double.parseDouble(texto.get(i).get(j)));
		}
		return aux;
	}
	...
	/**
	 * Carga una lista de reales tomada de la fila i, de palabras a partir de la columna j empezando en cero
	 * con j>0 y hasta el final de la fila
	 * Si la columna 0 no coincide con la etiqueta detiene la ejecución.
	 * @param i
	 * @param etiqueta
	 * @return
	 */
	public ArrayList<Double> cargaListaRealesColJ(int i, int j, String etiqueta){
		verificaEtiqueta(i, etiqueta);
		ArrayList<Double> aux = new ArrayList<Double>();
		for(int c=j; c<texto.get(i).size(); c++){
			aux.add(Double.parseDouble(texto.get(i).get(c)));
		}
		return aux;
	}
	...
	/**
	 * Carga una lista de enteros tomada de la fila i, de palabras a partir de la columna 1
	 * Si la columna 0 no coincide con la etiqueta detiene la ejecución.
	 * @param i
	 * @param etiqueta
	 * @return
	 */
	public ArrayList<Integer> cargaListaEnteros(int i, String etiqueta){
		verificaEtiqueta(i, etiqueta);
		ArrayList<Integer> aux = new ArrayList<Integer>();
		for(int j=1; j<texto.get(i).size(); j++){
			aux.add(Integer.parseInt(texto.get(i).get(j)));
		}
		return aux;
	}	
...
/**
 * EMPIEZAN METODOS DE ESCRITURA	
 */

	/**
	 * Escribe el texto de una matriz mat de reales de nfil filas y ncol columnas
	 * Cada fila de la matriz en una línea, los valores de una fila separados
	 * por el String separador sep.
	 */
	public String escribeMatrizReal(double[][] mat, String sep){
		StringBuilder sb = new StringBuilder();
		int nfil = mat.length;
		int ncol = mat[0].length;
		for(int ifil = 0; ifil<nfil; ifil++){
			for(int icol = 0; icol<ncol; icol++){
				sb.append(mat[ifil][icol] + sep);
			}
			sb.append("\n");
		}
		return sb.toString();
	...
	/**
	 * Escribe el texto de una etiqueta y una lista de Strings
	 * separado todo el separador sep
	 * Al final carga un salto de línea
	 */
	public String escribeEtiqYLista(String etiqueta, String[] lista, String sep){
		StringBuilder sb = new StringBuilder(etiqueta + sep);
		for(String s: lista){
			sb.append(s + sep);
		}
		sb.append("\n");
	...
	/**
	 * Escribe el texto de una etiqueta y una lista de reales
	 * separado todo por sep
	 * carga salto de línea al final
	 */
	public String escribeEtiqYListaReales(String etiqueta, double[] lista, String sep){
		StringBuilder sb = new StringBuilder(etiqueta + sep);
		for(double d: lista){
			sb.append(d + sep);
		}
	...
	/**
	 * Escribe el texto de una lista de reales
	 * separado todo por sep
	 * NO carga salto de línea al final
	 */
	public String escribeListaReales(double[] lista, String sep){
		StringBuilder sb = new StringBuilder();
		for(double d: lista){
			sb.append(d + sep);
		}
	...
	/**
	 * Escribe el texto de una etiqueta y un entero
	 * separado todo por sep
	 * Al final carga un salto de línea
	 */
	public String escribeEtiqYEntero(String etiqueta, int entero, String sep){
		return etiqueta + sep + entero + "\n";
	}
	
	
	/**
	 * Escribe el texto de una etiqueta y un real
	 * separado todo por sep
	 * Al final carga un salto de línea
	 */
	public String escribeEtiqYDouble(String etiqueta, double real, String sep){
		return etiqueta + sep + real + "\n";
	}	
	
	/**
	 * Escribe el texto de una etiqueta y un String
	 * separados por blancos
	 * Al final carga un salto de línea
	 */
	public String escribeEtiqYString(String etiqueta, String s, String sep){
		return etiqueta + sep + s + "\n";
	}
	
	
	/**
	 * Escribe el texto de una etiqueta y un valor SI/NO según el boolean
	 * separado todo el separador sep
	 * Al final carga un salto de línea
	 */
	public String escribeEtiqYSiNo(String etiqueta, boolean sino, String sep){
		StringBuilder sb = new StringBuilder(etiqueta + sep);
		if(sino){
			sb.append("SI");
		}else{
	...
	/**
	 * En un archivo de texto busca la línea que tiene como primera palabra 
	 * etiqueta y devuelve el resto de la línea separado en columnas por los espacios
	 * Si no encuentra la etiqueta devuelve null
	 */
	public ArrayList<String> devuelveLineaDeEtiqueta(String arch, String etiqueta){
	...
	public static void main(String[] args) { 
    	...
    	System.out.println("terminó prueba " + i);
    }
