...
public class ResolvedorGLPK_anterior implements ResolvedorLineal {	
	private glp_prob lp;
	...
	private void crearDimensionesProblema(DatosEntradaProblemaLineal entrada) {
		// System.out.println("ResolvedorGLPK_anterior::crearProblema");
		...
		// Create problem
		// System.out.println("ResolvedorGLPK_anterior::crearDimensionesProblema");
	...
	private void asignarCoeficientesProblema(DatosEntradaProblemaLineal entrada) {
		...
		// System.out.println("ResolvedorGLPK_anterior::asignarCoeficientesProblema");
		...
		if (!restricciones.isEmpty()) {
			...
			// System.out.println("ResolvedorGLPK_anterior::asignarCoeficientesProblema::glp_load_matrix");

			// GLPK.glp_load_matrix(lp, sizeCoefs, rowind, colind, coefs);
			GLPK.delete_intArray(colind);
			// System.out.println("ResolvedorGLPK_anterior::resolver::delete_intArray(colind)");
			GLPK.delete_doubleArray(coefs);
			// System.out.println("ResolvedorGLPK_anterior::resolver::delete_doubleArray(coefs)");
	...
	private void llamarSolver() {
		// System.out.println("ResolvedorGLPK_anterior::llamarSolver");
		 ...
		 * "ResolvedorGLPK_anterior::llamarSolver - CAGAMOS LA BASE INICIAL"); } }
		 */
		...
		if (Constantes.RES_GLP_RB) {
			...
			/*
			 * System.out.println("ResolvedorGLPK_anterior::check_kkt\t" + totalCalls + "\tPE\taeM\t"
			 * + ae_maxPE + "\taeI\t" + ae_indPE + "\treM\t" + re_maxPE + "\treI\t" +
			 * re_indPE); System.out.println("ResolvedorGLPK_anterior::check_kkt\t" + totalCalls +
			 * "\tPB\taeM\t" + ae_maxPB + "\taeI\t" + ae_indPB + "\treM\t" + re_maxPB +
			 * "\treI\t" + re_indPB); System.out.println("ResolvedorGLPK_anterior::check_kkt\t" +
			 * totalCalls + "\tDE\taeM\t" + ae_maxDE + "\taeI\t" + ae_indDE + "\treM\t" +
			 * re_maxDE + "\treI\t" + re_indDE);
			 * System.out.println("ResolvedorGLPK_anterior::check_kkt\t" + totalCalls + "\tDB\taeM\t"
...
//			if (Constantes.NIVEL_CONSOLA > 1) {
			//	System.out.println("ResolvedorGLPK_anterior::llamarSolver - CAGAMOS LA BASE INICIAL");
//			}
		}
		...
		if (retLP != 0) {
			...
			if (retLP != 0) {
//				if (Constantes.NIVEL_CONSOLA > 1) {
				//	System.out.println("ResolvedorGLPK_anterior::llamarSolver - cagamos segundo lp");
//				}
			}
		...
		if (ret != 0) {
//			if (Constantes.NIVEL_CONSOLA > 1) {
			//	System.out.println("ResolvedorGLPK_anterior::llamarSolver - cagamos el mip");
	//		}
			GLPK.glp_scale_prob(lp, GLPKConstants.GLP_SF_AUTO);
			resetBasis = true;
			GLPK.glp_std_basis(lp);
			...
			if (retLP != 0) {
//				if (Constantes.NIVEL_CONSOLA > 1) {
				//	System.out.println("ResolvedorGLPK_anterior::llamarSolver - cagamos segundo lp en mip");
//				}
			}
			ret = GLPK.glp_intopt(lp, parmI);
			if (ret != 0) {
//				if (Constantes.NIVEL_CONSOLA > 1) {
				//	System.out.println("ResolvedorGLPK_anterior::llamarSolver - cagamos el segundo mip");
//				}
			}
		...
		// System.out.println("ResolvedorGLPK_anterior::llamarSolve\t" + msg);
	//	ret = retLP;
	}
...
//		if ((escenario==1 || escenario==101 || escenario==10001 || escenario==10101 ) && paso==205) 
//		entrada.imprimir("d:/salidasLP/ENTRADA"+Integer.toString(escenario)+"paso"+paso);
		try {
			totalCalls++;
...
//				pbt.continuarContador("ResolverGLPK::obtenerSolucion");
				String archEnt = Constantes.RES_GLP_IMPRIMIR_LP_RUTA_CP + "\\casoOptimo.lp";
				DirectoriosYArchivos.siExisteElimina(archEnt);
				GLPK.glp_write_lp(lp, null, archEnt);
...
//				pbt.pausarContador("ResolverGLPK::obtenerSolucion");
				 ...
				 */
				// GLPK.glp_delete_prob(lp); // Solo elimina el problema
				// System.out.println("ResolvedorGLPK_anterior::resolver::glp_delete_prob");
...
//				if ((escenario==209010101 || escenario==209010001|| escenario==209020001 || escenario==209020101) && paso==186)  {
//					int ret = GLPK.glp_write_lp(lp, null, "d:\\salidasLP\\lp"+escenario+".lp");
//					sal.printSolucion("d:\\salidasLP\\SOLlp"+escenario+".lp");
//				}
//				if (escenario==1 && paso==5) {
//					int ret = GLPK.glp_write_lp(lp, null,
//						"d:\\esc1paso5.lp");
//					sal.printSolucion("d:\\salidaesc1paso4.sal");
//				}
//				sal.printSolucion("d:\\salidasLP\\salida" +escenario+paso+".sal");
				return sal;
			} else {
			//	if (ret != GLPKConstants.GLP_EROOT) {
...
// TODO IMPRIME		int ret = GLPK.glp_write_lp(lp, null,
//							dirInfactible + "infactGLPKesc" + escenario + "paso" + paso + ".lp");
	...
	public ResolvedorGLPK_anterior() {
		totalCalls = 0;
		problemCreated = 0;
		problemReused = 0;
		...
		if (Constantes.RES_GLP_RB) {
			if (Constantes.NIVEL_CONSOLA > 1)
				System.out.println("ResolvedorGLPK_anterior::REUTILIZA BASE");
		} else {
			if (Constantes.NIVEL_CONSOLA > 1)
				System.out.println("ResolvedorGLPK_anterior::NO REUTLIZA BASE");
		}
	}
}